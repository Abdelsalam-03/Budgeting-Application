 ...
 *   BudgetingView bv = new BudgetingView(() -> showDashboard());
 *   root.setCenter(bv.getView());
 */
public class BudgetingView {
    ...
    private VBox budgetCard(Budget b) {
        ...
        Label amtLbl = lbl(String.format("$%.2f spent of $%.2f", b.getSpent(), b.getAmount()),
                           13, FontWeight.NORMAL, SUB);

        ProgressBar bar = new ProgressBar(Math.min(b.getSpent() / b.getAmount(), 1.0));
    ...
    private void openForm(Budget budget) {
        ...
        if (isEdit) categoryBox.setValue(budget.getCategory());

        TextField amountField = new TextField();
        amountField.setPromptText("e.g. 300");
        styleField(amountField);
        if (isEdit) amountField.setText(String.valueOf(budget.getAmount()));

        ComboBox<String> periodBox = new ComboBox<>();
        YearMonth now = YearMonth.now();
        for (int i = -1; i <= 3; i++) periodBox.getItems().add(now.plusMonths(i).format(fmt));
        periodBox.setValue(isEdit ? budget.getPeriod().format(fmt) : now.format(fmt));
        ...
        saveBtn.addEventFilter(javafx.event.ActionEvent.ACTION, event -> {
            ...
            if (cat == null || cat.isEmpty()) {
                errorLbl.setText("Please select a category."); event.consume(); return;
            }
            if (amtTx.isEmpty()) {
                errorLbl.setText("Please enter an amount."); event.consume(); return;
            }

            double amount;
            try { amount = Double.parseDouble(amtTx); }
            catch (NumberFormatException ex) {
                errorLbl.setText("Amount must be a valid number."); event.consume(); return;
            }

            YearMonth selectedPeriod = YearMonth.parse(periodBox.getValue(), fmt);

            BudgetController.ControllerResult<Budget> result = isEdit
                    ? controller.updateBudget(budget.getId(), cat, amount, selectedPeriod)
                    : controller.saveBudget(cat, amount, selectedPeriod);
